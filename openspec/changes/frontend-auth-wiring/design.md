...
1. **刷新遇到暂时性失败（503 / 网络错误）时的用户侧处理**：直接提示错误，**不做重试**——不改动 `lib/api/client.ts` 的「401 → 刷新 → 重放」分支。已知取舍：此时调用方收到的仍是原始响应的 401，`message` 可能显示为"未认证"而非"服务不可用"；本期接受该偏差，若后续反馈误导再改请求层。
2. **`fetchCurrentUser` 的 `replayOn401: false` 暂时保留**：引导依赖它能收到 401 来区分"账号被停用"；已知代价是其它调用点（目前没有）不会自动续期，出现第二个调用点时再评估。
3. **React `act(...)` 告警按"不新增"验收**：该告警在本 change 之前就存在（`SystemStatusPage.test.tsx` 等既有测试同样产生，源于 TanStack Query 的状态更新落在 `act` 之外）。本 change 的新测试沿用仓库既有写法，**不新增告警类别**；彻底清理需要一并改写既有测试，记为**独立 follow-up**（不在本 change 范围）。据此任务 4.3 的验收口径为"较基线无新增告警"，而非"零告警"。
4. **启动引导做并发去重**：`main.tsx` 使用 StrictMode，effect 双调用会并发发两次 refresh，而刷新令牌是轮换制——两次并发会互相作废。去重采用"复用进行中的 Promise，结束后释放以便重试"，因此不影响引导失败后的重试入口。
